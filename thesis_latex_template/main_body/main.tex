...
\cabstract{
随着大数据与人工智能技术的迅速发展，工程设计、生物医学和科学计算等领域面临日益复杂的优化挑战，问题普遍呈现高维性、非线性和多模态等特征。尤其在高维数据分析场景中，特征空间的指数级膨胀引发"维数灾难"，使特征选择成为提升机器学习模型性能与可解释性的关键技术手段；而以仿真驱动为代表的昂贵优化问题则对求解方法的样本效率提出了更高要求。元启发式算法因其无需梯度信息、全局搜索能力强和适用范围广等优势，已成为求解上述复杂优化问题的主流方法之一。然而，现有算法在探索与利用的有效平衡、搜索策略的自适应调节以及昂贵优化场景下的样本利用效率等方面仍存在显著不足，制约了其在特征选择和工程优化等实际任务中的应用效果。本文围绕元启发式优化算法的改进与应用展开系统研究，提出了三种新型算法，并在全局优化、高维医学数据特征选择、图像分割和昂贵优化等多个应用场景中验证了其有效性。主要研究工作如下：
...
（2）受人类社会地位追求行为启发，提出了一种新型元启发式算法——基于状态的优化算法（SBO）。SBO模拟个体向高地位人物靠拢、学习和获取资源的社会行为模式，将其转化为高效的计算优化方法。在IEEE CEC 2017测试套件的10、30、50和100维问题上进行了全面对比分析。此外，SBO在3阈值水平的乳腺癌组织学图像多阈值分割应用中展现了优异的性能。

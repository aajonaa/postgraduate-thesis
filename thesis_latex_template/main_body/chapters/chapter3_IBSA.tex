...
\begin{equation}
\label{ibsa_eq_17}
V\left(X^j_i\left(t\right)\right)=\left|\frac{2}{π}*\mathrm{atan}\mathrm{}(\frac{π}{2}*X^j_i\left(t\right))\right|
\end{equation}
...
\begin{equation}
\label{ibsa_eq_19}
Fitness=α\cdot γ_R\left(N\right)+β\cdot \frac{\left|R\right|}{\left|N\right|}
\end{equation}

其中，$γ_R(\cdot )$表示计算所选特征子集$R$相关错误率的函数。$R$和$N$分别表示当前选择的特征和数据集中的总特征数。为了使评估与选择最小特征集和确保高精度的双重目标相一致，本文使用两个系数$α$和$β$，它们的作用是为适应度方程的每个组成部分分配适当的权重以实现最优平衡。

通过实验校准，系数$α$设定为0.5，建立了对两个目标之间重视程度的基线。相应地，$β$取$1-α$的值，根据本文的设置，最终值为0.95 \cite{ibsa_ref72}。该比率本质上决定了特征子集大小和分类精度之间的权衡，每个变量按其定义的系数加权，引导bIBSA朝着最有利的特征选择结果前进。

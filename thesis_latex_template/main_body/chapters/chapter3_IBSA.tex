...
面对上述挑战，元启发式算法由于无需梯度信息、实现灵活，已在此类复杂问题的求解中积累了大量成功案例\cite{ibsa_ref24}。进化算法、群智能优化等路线在各自擅长的场景中均有不俗表现\cite{ibsa_ref25,ibsa_ref29,ibsa_ref33,ibsa_ref37}。但一旦面对高维特征选择任务，现有方法普遍暴露出局部开发不足、参数调整繁琐以及收敛速度受限等问题\cite{ibsa_ref41,ibsa_ref50,ibsa_ref51}。
...
总体来看，BSA通过精简的参数设计表现出向无参数算法演进的潜力。然而，在处理高维复杂问题时，BSA的局限性逐渐暴露：首先，其交叉模式的随机切换难以适应种群进化的动态过程\cite{ibsa_ref63}；其次，算法未充分利用精英解的引导信息，削弱了其在高质量区域的搜索深度\cite{ibsa_ref67}；此外，历史种群的盲目更新也导致搜索缺乏明确的方向。为此，本章提出精英引导变异策略，通过优化历史信息的利用方式并改进交叉机制，在保持全局探索能力的同时，显著提升了算法的局部开发精度。
...
实验结果表明，IBSA在CEC 2017全场景测试中在全局最优解搜索上表现出色，特别是在复杂问题下的稳定性尤为突出。同时，在医学特征选择任务中，bIBSA在实现数据大幅压缩的同时保持了业内领先的分类性能。研究表明，所提出的精英引导范式为处理现代大规模组合优化问题提供了一种有效的方法。

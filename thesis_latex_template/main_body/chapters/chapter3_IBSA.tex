...
V型函数将连续位置映射为翻转概率。若该概率表明翻转可能改善适应度，则执行翻转（公式\eqref{ibsa_eq_18}）；否则保持原值。这一机制使种群个体能够在二值空间的顶点间有效探索。
\begin{equation}
\label{ibsa_eq_17}
...
为了全面验证改进方案的有效性，本节开展了由标准连续函数测试与实际医学特征选择组成的实验。其中，连续函数测试用于验证改进策略对搜索性能的增益，医学数据集实验则侧重考察算法在离散工程任务中的泛化表现。
...
实验结果表明，IBSA在CEC 2017基准测试中取得了较好的寻优精度，在复杂函数上的稳定性尤为突出。在医学特征选择任务中，bIBSA在大幅压缩特征空间的同时保持了较高的分类精度，为处理大规模组合优化问题提供了一种可行的思路。

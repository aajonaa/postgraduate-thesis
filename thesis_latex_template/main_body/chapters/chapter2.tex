...
本研究关注的高维特征筛选与医学影像分割，本质上均可转化为特定空间下的参数寻优问题。前者致力于在离散组合域内筛选最优特征子集，后者则旨在连续灰度空间中确定最优分割阈值。
...
\end{equation}
其中，$Q_t(a)$为臂$a$的估计收益均值，$N_t(a)$为臂$a$被选择的次数，$c$为探索系数。UCB策略的核心在于：$Q_t(a)$鼓励选择历史表现较好的臂（利用），探索项$c \sqrt{\ln t / N_t(a)}$则鼓励尝试被选次数较少的臂（探索）。第五章以此为蓝本，构建了赌博机驱动的自适应调度机制，通过将异构搜索模式映射为博弈臂，实现对搜索策略的实时动态分配。

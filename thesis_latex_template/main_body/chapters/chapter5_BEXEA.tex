...
\section{引言}
在处理高昂评估代价优化问题（Expensive Optimization Problems, EOPs）时，单次目标函数评估往往需要调用计算流体力学或有限元分析等复杂数值仿真，计算开销极大\cite{liu2023airfoils,kumar2024optimization}。受计算资源限制，函数评估次数通常只能控制在几百至几千次，传统进化算法难以在如此有限的评估预算内获得高质量解\cite{he2023review}。代理辅助进化算法（Surrogate-Assisted Evolutionary Algorithms, SAEAs）通过构建样本档案并训练代理模型，以低成本近似估计替代高昂的真实评估，从而在有限评估预算内提升搜索效率\cite{kudela2022recent,jin2018data}。

代理模型、搜索策略和采样准则构成了SAEA框架的核心。尽管RBF、Kriging等模型已被广泛使用\cite{regis2013evolutionary,sun2017surrogate,buche2005accelerating,liu2013gaussian,loshchilov2010comparison,guo2018heterogeneous,zhou2005study,chavez2016polynomial}，但现有搜索机制仍较为僵化，往往依赖预设的固定模式\cite{khaldi2024surrogate,stanovov2023surrogate,mallipeddi2015evolving,lv2023multi,yu2025surrogate,li2020surrogate,lin2024surrogate}，在面对不同函数特性时缺乏自适应调节能力。同时，EI等常用采样准则虽然能够兼顾探索与开发\cite{mockus1994application,jones1998efficient}，但其决策过程缺少直观解释，难以说明候选解被优先评估的具体依据，因此在实际工程应用中不利于建立方法可信度。
...
为验证BEXEA的性能，本章设置了两类验证场景：首先利用标准昂贵优化基准函数考察算法的全局搜索性能，随后通过油藏产量优化算例检验算法在高保真模拟环境中的应用效果。油藏产量优化具有仿真耗时长、非线性程度高和变量耦合复杂等典型特征，是评估算法样本利用效率的重要应用场景。
...
\end{cases}
\end{equation}
其中，$Q_{FEs}(a)$表示第$FEs$次真实评估时策略$a$的价值估计，$\beta \in (0,1]$为指数近因加权平均的更新系数。较大的$\beta$使价值估计对近期反馈更加敏感，较小的$\beta$则保留更多历史经验。当搜索进程停滞（连续多次真实评估未发现优解）时，算法通过提高探索率尝试非贪心策略，以重新激活全局搜索。这种动态调节机制更适合复杂问题在不同搜索阶段的需求差异。
...
对于每个候选解，算法从历史档案中选取$\mathcal{K}$个近邻估计局部均值与标准差，并计算期望改进（EI），以维持对潜在优选区域的偏好。同时，本文计算候选解与近邻之间的特征差异分布。借鉴SHAP等特征重要性分析方法的思路，这些差异被作为刻画局部维度敏感性的辅助信息。

通过计算特征重要性分布的Shannon熵，可以量化预测归因的不确定性。熵值较高意味着预测驱动因素较为分散，而熵值较低则表明归因依据更为集中。

最终的EU评价值综合考虑EI与反向熵值，优先选择既具有改进潜力又具有清晰归因依据的候选区域。这种采样策略有助于提高搜索的针对性，并为工程决策提供更明确的参考。在BEXEA中，EU准则用于筛选候选解，同时也被纳入赌博机奖励机制，引导搜索策略向更具解释性的方向演化。
...
在CEC 2022基准函数集测试中，BEXEA的平均排名位居第一。Wilcoxon秩和检验表明，BEXEA在多数函数上占优，这主要得益于MAB机制对策略权重的实时调整，以及EU准则通过特征归因分析提升了样本评估效率。
...
在高维场景（表\ref{t:bexea_CEC05_Table}）中，BEXEA的性能优势仍然明显。在100维环境下，MAB驱动的策略切换机制能够较好地应对复杂搜索景观，在维持种群多样性的同时降低陷入局部极值的风险，说明该方法具有良好的高维可扩展能力。
...
表\ref{t:bexea_Oil_Exp_Table}显示，BEXEA在核心经济指标上优于所有对比算法。其平均净现值（NPV）相较次优算法有明显提升，同时在求解一致性和运行效率方面也表现出显著优势。
...
实验结果表明，BEXEA在不同维度的标准基准函数集测试中均展现出较强的寻优能力。在油藏产量优化这一典型工程应用中，算法的收敛速度与求解精度进一步验证了其处理高维复杂高昂代价优化问题的有效性。消融实验则从不同角度分析了自适应策略演化和可解释不确定性采样准则对整体性能的提升作用。BEXEA在增强搜索自适应性的同时，也通过特征归因分析提高了采样决策的可解释性，为高昂评估代价优化问题提供了一种可行的求解框架。

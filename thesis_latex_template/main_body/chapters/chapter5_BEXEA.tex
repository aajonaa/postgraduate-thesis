...
生成的候选解经可解释不确定性（EU）准则评估，评分靠前的样本获得评估预算。在获取真实反馈后，算法自动更新策略的价值估计并调整后续偏好。相比人为设定的固定规则，BEXEA在平衡勘探与开发方面表现出更强的适应性。算法整体流程如图\ref{fig:bexea_BEXEA}所示，机制细节见图\ref{fig:bexea_MAB}与图\ref{fig:bexea_EU}。
...
收敛曲线（图\ref{fig:bexea_CEC22_CC}）显示，在F4、F6等挑战性函数上，BEXEA能更快逼近优值，体现了自适应搜索在调控勘探与开发方面的灵活性。散点分布（图\ref{fig:bexea_CEC22_Plots}）表明BEXEA结果分布集中，算法具备良好的稳定性。
...
实验结果表明，BEXEA在不同维度的标准测试函数中均具备较强的寻优能力。在油藏产量优化这一典型工程算例中，算法的收敛速度与求解精度进一步验证了其处理高维复杂工程问题的效能。消融实验则从多角度剖析了自适应策略演化及多准则采样对整体性能的增益效果。BEXEA在提升搜索自适应性的同时增强了决策过程的可物理感知性，为昂贵优化问题的求解提供了有效的技术路径。

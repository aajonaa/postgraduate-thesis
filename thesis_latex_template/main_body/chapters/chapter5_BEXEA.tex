...
针对上述挑战，本章从搜索策略自适应演化与采样决策可解释性两个维度展开研究。一方面，将双臂赌博机机制引入搜索框架，利用实时环境反馈引导搜索模式的动态切换\cite{slivkins2019introduction}；另一方面，借鉴SHAP思想\cite{nohara2022explanation}构建特征重要性评估组件，提出一种兼顾优化潜能与决策物理依据的可解释不确定性准则。
...
收敛轨迹（图\ref{fig:bexea_Oil_Exp_Figure}）反映了BEXEA良好的初始搜索效能。在优化中后期，得益于MAB机制对搜索停滞状态的动态响应，算法能持续向更优区域逼近，在有限评估预算内获得了较理想的经济效益。

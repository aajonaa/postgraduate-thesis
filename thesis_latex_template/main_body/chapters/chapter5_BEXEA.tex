...
\section{引言}
处理昂贵优化问题（Expensive Optimization Problems, EOPs）时，单次目标函数评估常涉及计算流体力学或有限元分析等复杂数值模拟，计算开销极大\cite{liu2023airfoils,kumar2024optimization}。受计算资源限制，评估次数通常被限制在几百至几千次，传统进化算法在此时限内难以达到理想精度\cite{he2023review}。代理辅助进化算法（Surrogate-Assisted Evolutionary Algorithms, SAEAs）通过构建样本档案并训练代理模型，利用低成本近似估算替代高昂的真实评估，在有限预算内改善了搜索效率\cite{kudela2022recent,jin2018data}。

代理模型、搜索策略及填充准则是SAEA框架的核心。尽管RBF、Kriging等模型应用广泛\cite{regis2013evolutionary,sun2017surrogate,buche2005accelerating,liu2013gaussian,loshchilov2010comparison,guo2018heterogeneous,zhou2005study,chavez2016polynomial}，但现有的搜索机制仍显僵化，多依赖预设的固定模式\cite{khaldi2024surrogate,stanovov2023surrogate,mallipeddi2015evolving,lv2023multi,yu2025surrogate,li2020surrogate,lin2024surrogate}，面对变化的函数特性时缺乏自适应调节能力。同时，EI等常用填充准则虽然能平衡探索与开发\cite{mockus1994application,jones1998efficient}，但其决策过程缺乏直观的物理解释，难以阐明候选解优选的具体依据，在实际工程应用中存在信任度构建困难的问题。

针对上述挑战，本章从搜索策略自适应演化与采样决策可解释性两个维度展开研究。一方面，将双臂赌博机机制引入搜索框架，利用实时环境反馈引导搜索模式的动态切换\cite{slivkins2019introduction}；另一方面，借鉴SHAP思想\cite{nohara2022explanation}构建特征重要性评估组件，提出一种兼顾优化潜能与决策依据的可解释不确定性准则。

为验证BEXEA的性能，本章构建了双重验证场景：首先利用标准昂贵优化基准函数考察算法的全局搜索效能，随后通过油藏产量优化算例检验算法在实际高保真模拟环境下的应用表现。油藏工程优化具有仿真耗时长、非线性程度高、变量耦合复杂等典型特征，是评估算法样本效率的理想平台。

本章主要研究工作包括：构建了适用于SAEAs的赌博机自适应搜索框架，实现了算法搜索行为的在线调节；提出了可解释不确定性准则，通过量化特征归因提升了采样决策的透明度；整合形成了BEXEA算法，并在多类复杂任务中验证了其有效性。

5.2节详细阐述BEXEA的算法构造及搜索机制；5.3节给出基准测试与工程应用的实验分析；5.4节对本章工作进行概括。

\section{BEXEA算法及其搜索机制}

本节介绍BEXEA的整体逻辑框架，并解析赌博机驱动的自适应搜索机制与可解释不确定性准则。
\subsection{算法框架}

BEXEA分为初始化与主循环两个阶段。初始化过程采用拉丁超立方采样确定初始种群，并依据评估结果构建RBF代理模型；同时，预设EDA与QPSO两种策略的初始奖励、探索率等参数。

主循环执行期间，算法同步更新代理模型以捕捉最新样本信息。随后利用带停滞触发的$\epsilon$-贪心机制，在侧重宏观分布探索的EDA与专注于最优解精细挖掘的QPSO之间动态切换。这种互补组合为赌博机机制提供了灵活的优化路径。

生成的候选解经可解释不确定性（EU）准则评估，评分靠前的样本获得评估预算。在获取真实反馈后，算法自动更新策略的价值估计并调整后续偏好。相比人为设定的固定规则，BEXEA在平衡探索与开发方面表现出更强的适应性。算法整体流程如图\ref{fig:bexea_BEXEA}所示，机制细节见图\ref{fig:bexea_MAB}与图\ref{fig:bexea_EU}。
...
在线策略选择机制能够利用优化过程中的即时反馈动态调控探索与开发行为\cite{xie2023surrogate,gong2024surrogate}。通过将$\epsilon$-贪心MAB嵌入SAEA框架，本章实现了EDA与QPSO策略的在线切换，并辅以停滞检测机制增强算法脱离局部极值的能力。

策略选择被抽象为二臂赌博机问题。在每次迭代中，算法依据策略评分和探索概率执行动作。获得评估结果后，计算即时奖励并更新策略价值估计$Q(a)$。
...
\end{cases}
\end{equation}
其中$\epsilon$设为0.1至0.3，通过维持适度探索防止算法过早收敛至特定策略。

奖励函数综合考虑了适应度改进量与策略特有的搜索贡献。EDA阶段侧重于奖励提升种群多样性的行为，QPSO阶段则将解的可解释性评分纳入考量。这种多准则奖励机制有助于算法识别不同策略在特定搜索阶段的潜在价值。

价值更新采用指数近因加权平均，兼顾历史表现与近期收益。当搜索进程停滞（连续多次迭代未发现优解）时，算法通过上调探索率尝试非贪心策略，以激活全局搜索活力。这种动态调节机制较传统固定参数更契合复杂问题不同阶段的搜索需求。

图\ref{fig:bexea_MAB}展示了该过程。算法通过“决策-评估-反馈”循环，自动演化出匹配当前搜索景观的模式，在收敛效率与解质量之间寻求平衡。
...
EU准则融合贝叶斯优化与可解释人工智能理论，为采样决策提供兼顾预测增量与决策透明度的衡量标准。

对于候选解，算法选取历史档案中的$\mathcal{K}$个近邻估计局部均值与标准差，计算期望改进（EI），以保持对潜在优选区域的偏好。同时，本章计算候选解与近邻的特征差异分布。受SHAP原理启发，这些差异被视为局部维度的重要性表征。

通过计算特征重要性分布的Shannon熵，量化预测归因的不确定性。高熵值意味着预测驱动因素模糊，低熵点则表明归因依据明确。

最终的EU评价值结合了EI与反向熵值：优先选择具有改进潜力且归因清晰的候选区域。这种采样策略有助于提升搜索的针对性，并为工程决策提供支撑依据。在BEXEA中，EU准则不仅用于候选解筛选，还作为赌博机奖励机制的组成部分，引导搜索策略向更具解释力的方向演化。
...
本节通过昂贵优化基准问题评估BEXEA性能，随后将其应用于油藏产量优化，最后利用消融与参数实验剖析核心机制。

\subsection{实验设置}

本研究采用了两组基准测试。其一为IEEE CEC 2022测试集（$d=20$），用于考察算法在标准维度下的优化能力；其二为高维场景测试套件（$d=50$和$d=100$），涵盖单峰、多峰旋转及混合函数。统计分析采用Friedman检验与Wilcoxon秩和检验（显著性水平$\alpha=0.05$）。

对比算法包括TSA-BFEA、KTA2、DESO等七种SAEAs，参数执行原作者推荐设置。BEXEA衰减因子设为0.3，近邻数为8。所有任务函数评估上限为1000次，独立运行20次。

\subsection{油藏问题描述}

将算法应用于油藏产量优化任务。核心目标是寻找最优井控方案，在权衡注水与生产成本的前提下实现净现值（NPV）最大化。

实验采用经典的Egg模型。系统包含8口注水井与4口生产井，决策空间为40维。单次油藏模拟涉及多相流偏微分方程求解，计算开销极大。在200次评估预算内寻找优控制方案，对算法的样本利用效率提出了严苛要求。
...
如图\ref{fig:bexea_oil_problem}所示，井控方案经高保真模拟器处理输出产量数据，由经济模型折算为NPV。该流程体现了昂贵优化问题高开销、非线性和高度耦合的特征。

\subsection{实验结果与分析}

分析BEXEA在基准函数与工程算例中的表现。
...
CEC 2022测试中，BEXEA平均排名第一。Wilcoxon检验表明BEXEA在多数函数上占优，这得益于MAB机制对策略权重的实时动态调整，以及EU准则通过特征归因分析改善了样本评估效率。
...
收敛曲线（图\ref{fig:bexea_CEC22_CC}）显示，在F4、F6等挑战性函数上，BEXEA能更快逼近优值，体现了自适应搜索在调控探索与开发方面的灵活性。散点分布（图\ref{fig:bexea_CEC22_Plots}）表明BEXEA结果分布集中，算法具备良好的稳定性。

在高维场景（表\ref{t:bexea_CEC05_Table}）中，BEXEA的性能优势依然显著。在100维环境下，MAB驱动的策略切换机制能有效应对复杂搜索景观，在维持种群多样性的同时降低了陷入局部最优的风险，证明了其处理高维任务的扩展性。
...
消融实验验证了自适应搜索机制的必要性。表\ref{t:bexea_MAB_Table}结果显示，BEXEA性能显著优于各单一策略版本。MAB驱动的灵活切换机制是算法跨越不同搜索景观、实现多阶段性能最优的关键所在。
...
采样准则消融分析见表\ref{t:bexea_EU_Table}。缺少采样筛选环节会导致算法性能出现下滑。EU准则与代理模型预测相结合，不仅在多项任务中实现了优势互补，整体排名亦居首位。这表明引入特征重要性引导采样决策可有效优化评估预算的分配，并增强优化过程的可解释性。
...
超参数$\beta$与$\mathcal{K}$的配置对算法稳定性具有重要影响。衰减因子决定了策略更新对近期搜索表现的敏感程度，而近邻数则直接影响局部不确定性的估计精度。图\ref{fig:bexea_Beta}与图\ref{fig:bexea_K}展示了不同参数下的性能对比。综合权衡收敛速度与求解精度，核心设置在多数测试中均表现均衡。
...
本实验通过MRST工具箱构建仿真环境，对井控问题开展数值仿真实验。
...
表\ref{t:bexea_Oil_Exp_Table}结果显示，BEXEA在核心经济指标上优于对比算法。其平均NPV较次优算法有显著提升，且在求解一致性与运行效率方面具备明显优势。
...
收敛轨迹（图\ref{fig:bexea_Oil_Exp_Figure}）反映了BEXEA良好的初始搜索效能。在优化中后期，得益于MAB机制对搜索停滞状态的动态响应，算法能持续向更优区域逼近，在有限评估预算内获得了较优的经济收益。

\section{本章小结}
本章针对代理辅助昂贵优化中策略僵化及采样准则黑箱化等不足，构建了BEXEA算法。该方法通过融合多臂赌博机的自适应机制与可解释人工智能理论，实现了对搜索资源分配与采样决策的协同调控。

实验结果表明，BEXEA在不同维度的标准测试函数中均具备较强的寻优能力。在油藏产量优化这一典型工程算例中，算法的收敛速度与求解精度进一步验证了其处理高维复杂工程问题的效能。消融实验则从多角度剖析了自适应策略演化及多准则采样对整体性能的增益效果。BEXEA在提升搜索自适应性的同时，通过特征归因分析增强了采样决策的可解释性，为昂贵优化问题的求解提供了一条有效路径。

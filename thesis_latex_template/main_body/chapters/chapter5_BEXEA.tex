...
为验证BEXEA的性能，本章设置了两类验证场景：首先利用标准昂贵优化基准函数考察算法的全局搜索性能，随后通过油藏产量优化算例检验算法在高保真模拟环境中的应用效果。油藏工程优化具有仿真耗时长、非线性程度高和变量耦合复杂等典型特征，是评估算法样本利用效率的理想平台。

本章的主要工作包括：构建适用于SAEAs的赌博机自适应搜索框架，实现搜索行为的在线调节；提出可解释不确定性准则，通过量化特征归因提升采样决策的透明度；在此基础上形成BEXEA算法，并在多类复杂任务中验证其有效性。
...
在主循环过程中，算法同步更新代理模型以吸收最新样本信息。随后利用带停滞触发机制的$\epsilon$-贪心策略，在侧重宏观分布探索的EDA与专注于精细局部搜索的QPSO之间进行动态切换。这种互补组合为赌博机机制提供了更灵活的优化路径。

生成的候选解经可解释不确定性（EU）准则评估后，评分较高的样本将获得真实评估预算。取得反馈后，算法自动更新策略价值估计并调整后续选择偏好。与人为设定的固定规则相比，BEXEA在平衡探索与开发方面具有更强的适应性。算法整体流程如图\ref{fig:bexea_BEXEA}所示，具体机制见图\ref{fig:bexea_MAB}与图\ref{fig:bexea_EU}。
...
奖励函数综合考虑适应度改进量与策略特有的搜索贡献。EDA阶段更重视能够提升种群多样性的行为，QPSO阶段则将解的可解释性评分纳入考量。这样的多准则奖励设计有助于算法识别不同策略在不同搜索阶段的潜在价值。

价值更新采用指数近因加权平均，在兼顾历史表现的同时强调近期收益。当搜索进程停滞（连续多次迭代未发现优解）时，算法通过提高探索率尝试非贪心策略，以重新激活全局搜索。这种动态调节机制比固定参数设置更符合复杂问题在不同阶段的搜索需求。

图\ref{fig:bexea_MAB}展示了这一过程。算法通过“决策—评估—反馈”循环，逐步形成与当前搜索景观相匹配的策略偏好，在收敛效率与解质量之间取得平衡。
...
EU准则融合贝叶斯优化与可解释人工智能思想，为采样决策提供兼顾预测增量和决策透明度的评价标准。
...
最终的EU评价值综合考虑EI与反向熵值，优先选择既具有改进潜力又具有清晰归因依据的候选区域。这种采样策略有助于提高搜索的针对性，并为工程决策提供更明确的参考。在BEXEA中，EU准则不仅用于筛选候选解，也作为赌博机奖励机制的一部分，引导搜索策略向更具解释性的方向调整。
...
在CEC 2022测试中，BEXEA的平均排名位居第一。Wilcoxon检验表明，BEXEA在多数函数上占优，这主要得益于MAB机制对策略权重的实时调整，以及EU准则通过特征归因分析提高了样本评估效率。
...
在高维场景（表\ref{t:bexea_CEC05_Table}）中，BEXEA的性能优势仍然明显。在100维环境下，MAB驱动的策略切换机制能够较好地应对复杂搜索景观，在维持种群多样性的同时降低陷入局部最优的风险，说明该方法具有较好的高维扩展性。
...
采样准则的消融分析见表\ref{t:bexea_EU_Table}。结果表明，缺少采样筛选环节会导致算法性能下降。EU准则与代理模型预测相结合后，不仅在多项任务中形成互补，整体排名也位居前列。这说明利用特征重要性引导采样决策，能够更有效地分配评估预算，并增强优化过程的可解释性。
...
表\ref{t:bexea_Oil_Exp_Table}显示，BEXEA在核心经济指标上优于对比算法。其平均NPV相较次优算法有明显提升，同时在求解一致性和运行效率方面也具有优势。
...
收敛轨迹（图\ref{fig:bexea_Oil_Exp_Figure}）表明，BEXEA在搜索初期便能较快形成有效的改进趋势。进入中后期后，得益于MAB机制对搜索停滞状态的动态响应，算法仍能持续向更优区域逼近，并在有限评估预算内取得较好的经济收益。

\section{本章小结}
本章针对代理辅助昂贵优化中搜索策略僵化和采样准则缺乏解释性等问题，构建了BEXEA算法。该方法通过融合多臂赌博机的自适应机制与可解释人工智能思想，实现了搜索资源分配与采样决策的协同调控。

实验结果表明，BEXEA在不同维度的标准测试函数上均表现出较强的寻优能力。在油藏产量优化这一典型工程算例中，算法的收敛速度与求解精度进一步验证了其处理高维复杂工程问题的有效性。消融实验则从不同角度分析了自适应策略演化和多准则采样对整体性能的提升作用。总体来看，BEXEA在增强搜索自适应性的同时，也通过特征归因分析提高了采样决策的可解释性，为昂贵优化问题提供了一种有效的求解思路。

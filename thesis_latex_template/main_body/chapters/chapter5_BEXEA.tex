...
5.2节详细阐述BEXEA的算法构造及搜索机制；5.3节给出基准测试与工程应用的实验分析；5.4节对本章工作进行概括。
...
生成的候选解经可解释不确定性（EU）准则评估后，评分较高的样本将获得真实评估预算。取得反馈后，算法自动更新策略价值估计并调整后续选择偏好。与人为设定的固定规则相比，BEXEA在平衡探索与开发方面表现出更强的适应性。算法整体流程如图\ref{fig:bexea_BEXEA}所示，具体机制见图\ref{fig:bexea_MAB}与图\ref{fig:bexea_EU}。
...
EU准则融合贝叶斯优化与可解释人工智能思想，为采样决策提供兼顾预测增量和决策透明度的评价依据。
...
本节首先通过昂贵优化基准问题评估BEXEA性能，随后将其应用于油藏产量优化，并结合消融实验和参数实验分析核心机制。
...
如图\ref{fig:bexea_oil_problem}所示，井控方案经高保真模拟器计算后输出产量数据，再由经济模型折算为NPV。该流程体现了昂贵优化问题高开销、强非线性和高度耦合的特点。

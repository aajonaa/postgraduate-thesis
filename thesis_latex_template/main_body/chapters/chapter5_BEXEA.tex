...
\section{引言}
在处理高昂评估代价优化问题（Expensive Optimization Problems, EOPs）时，单次目标函数评估往往需要调用计算流体力学或有限元分析等复杂数值仿真，计算开销较高\cite{liu2023airfoils,kumar2024optimization}。受计算资源限制，函数评估次数通常只能控制在几百至几千次，传统进化算法难以在如此有限的评估预算内获得高质量解\cite{he2023review}。代理辅助进化算法（Surrogate-Assisted Evolutionary Algorithms, SAEAs）通过构建样本档案并训练代理模型，以低成本近似估计替代高昂的真实评估，从而在有限评估预算内提升搜索效率\cite{kudela2022recent,jin2018data}。

代理模型、搜索策略和采样准则构成了SAEA框架的核心。尽管RBF、Kriging等模型已被广泛使用\cite{regis2013evolutionary,sun2017surrogate,buche2005accelerating,liu2013gaussian,loshchilov2010comparison,guo2018heterogeneous,zhou2005study,chavez2016polynomial}，但现有搜索过程多依赖预设规则\cite{khaldi2024surrogate,stanovov2023surrogate,mallipeddi2015evolving,lv2023multi,yu2025surrogate,li2020surrogate,lin2024surrogate}，在面对不同函数特性时自适应调节能力不足。同时，EI等常用采样准则虽然能够兼顾探索与开发\cite{mockus1994application,jones1998efficient}，但其决策过程缺少直观解释，难以说明候选解被优先评估的具体依据，因此在实际工程应用中会影响方法可信度。

针对上述挑战，本章从搜索策略自适应演化和采样决策可解释性两个方面展开研究。一方面，将多臂赌博机机制引入搜索框架，利用实时环境反馈引导搜索模式的动态切换\cite{slivkins2019introduction}；另一方面，借鉴Shapley Additive Explanations（SHAP）思想\cite{nohara2022explanation}构建特征重要性评估组件，提出兼顾优化潜力与决策依据的可解释不确定性准则。

为验证BEXEA的性能，本章设置了两类验证场景：首先利用标准昂贵优化基准函数考察算法的全局探索性能，随后通过油藏产量优化算例检验算法在高保真模拟环境中的应用效果。油藏产量优化具有仿真耗时长、非线性程度高和变量耦合复杂等典型特征，是评估算法样本利用效率的重要应用场景。

本章的主要工作包括：构建适用于SAEAs的赌博机自适应搜索框架，实现搜索行为的在线调节；提出可解释不确定性准则，通过量化特征归因增强采样决策的可解释性；在此基础上形成BEXEA算法，并在多类复杂任务中验证其有效性。
...
BEXEA包括初始化和主循环两个阶段。初始化阶段采用拉丁超立方采样生成初始种群，并依据评估结果构建RBF代理模型；同时，为分布估计算法（Estimation of Distribution Algorithm, EDA）与量子粒子群优化算法（Quantum-behaved Particle Swarm Optimization, QPSO）两种策略设置初始奖励和探索率等参数。

在主循环过程中，算法同步更新代理模型以吸收最新样本信息。随后利用带停滞触发机制的$\epsilon$-贪心策略，在侧重全局分布探索的EDA与专注于局部开发的QPSO之间进行动态切换。这种互补组合为赌博机机制提供了更灵活的优化路径。

生成的候选解经可解释不确定性（Explainable Uncertainty, EU）准则评估后，评分较高的样本将优先获得真实评估机会。取得反馈后，算法自动更新策略价值估计并调整后续选择偏好。与人为设定的固定规则相比，BEXEA在平衡探索与开发方面表现出更强的适应性。算法整体流程如图\ref{fig:bexea_BEXEA}所示，具体机制见图\ref{fig:bexea_MAB}与图\ref{fig:bexea_EU}。
...
策略选择被抽象为二臂赌博机问题。在每次决策中，算法根据策略评分和探索概率执行动作；获得评估结果后，再计算即时奖励并更新策略价值估计$Q(a)$。

本章采用的$\epsilon$-贪心逻辑为：
\begin{equation}
a_{FEs} =
\begin{cases}
\argmax\limits_a Q_{FEs}(a) & \text{with probability } 1-\epsilon \\
\text{random strategy} & \text{with probability } \epsilon
\end{cases}
\end{equation}
其中$\epsilon$设为0.1至0.3，通过维持适度探索避免算法过早固化到某一策略。

为使奖励反馈同时刻画真实优化收益与策略特有贡献，本文定义第$FEs$次真实评估后的即时奖励为
\begin{equation}
r_{FEs}(a_{FEs}) = \lambda \Delta f_{FEs} + (1-\lambda)c_{FEs}(a_{FEs}),
\end{equation}
\begin{equation}
\Delta f_{FEs} = \max\left\{0,\frac{f^\star_{FEs-1}-f(\mathbf{x}_{FEs})}{\left|f^\star_{FEs-1}\right|+\varepsilon_f}\right\}
\end{equation}
\begin{equation}
c_{FEs}(a_{FEs}) =
\begin{cases}
\displaystyle \frac{\mathcal{D}_{FEs}-\mathcal{D}_{FEs-1}}{\left|\mathcal{D}_{FEs-1}\right|+\varepsilon_d}, & a_{FEs}=a_{\mathrm{EDA}}, \\[10pt]
\displaystyle \frac{\mathrm{EU}(\mathbf{x}_{FEs})-\mathrm{EU}_{\min}}{\mathrm{EU}_{\max}-\mathrm{EU}_{\min}+\varepsilon_u}, & a_{FEs}=a_{\mathrm{QPSO}}.
\end{cases}
\end{equation}
其中，$r_{FEs}(a_{FEs})$表示所选策略$a_{FEs}$在第$FEs$次真实评估后获得的即时奖励，$\Delta f_{FEs}$表示归一化适应度改进量，$f^\star_{FEs-1}$为当前历史最优适应度值，$f(\mathbf{x}_{FEs})$为候选解$\mathbf{x}_{FEs}$的真实适应度值。$c_{FEs}(a_{FEs})$表示策略特有的搜索贡献，其中$\mathcal{D}_{FEs}$为与第$FEs$次真实评估对应的种群多样性指标，用于刻画EDA对全局探索的促进作用；$\mathrm{EU}(\mathbf{x}_{FEs})$为QPSO生成候选解的可解释不确定性得分，$\mathrm{EU}_{\min}$与$\mathrm{EU}_{\max}$分别为当前候选集上该得分的最小值与最大值。$\lambda \in [0,1]$为奖励平衡系数，$\varepsilon_f$、$\varepsilon_d$与$\varepsilon_u$为防止分母为零的极小正数。

策略价值估计采用指数近因加权平均进行更新，即
\begin{equation}
Q_{FEs+1}(a)=
\begin{cases}
(1-\beta)Q_{FEs}(a) + \beta r_{FEs}(a_{FEs}), & a=a_{FEs}, \\
Q_{FEs}(a), & a \neq a_{FEs},
\end{cases}
\end{equation}
其中，$Q_{FEs}(a)$表示第$FEs$次真实评估时策略$a$的价值估计，$\beta \in (0,1]$为指数近因加权平均的更新系数。较大的$\beta$使价值估计对近期反馈更加敏感，较小的$\beta$则保留更多历史经验。当搜索进程停滞（连续多次真实评估未发现优解）时，算法通过提高探索率尝试非贪心策略，以重新激活全局探索。这种动态调节机制更适合复杂问题在不同搜索阶段的需求差异。

图\ref{fig:bexea_MAB}展示了这一过程。算法通过“决策—评估—反馈”循环，逐步形成与当前问题特征相匹配的策略偏好，在收敛效率与解质量之间实现协调。
...
EU准则融合贝叶斯优化与可解释人工智能思想，为采样决策提供兼顾预测改进潜力和归因清晰度的评价依据。

对于每个候选解，算法从历史档案中选取$\mathcal{K}$个近邻估计局部均值与标准差，并计算期望改进（EI），以保持对潜在优良区域的关注。同时，本文计算候选解与近邻之间的特征差异分布。借鉴SHAP等特征重要性分析方法的思路，这些差异被作为刻画局部维度敏感性的辅助信息。

通过计算特征重要性分布的Shannon熵，可以量化预测归因的不确定性。熵值较高表示归因分布较为分散，熵值较低则表明主要影响维度更为集中。

最终的EU评价值综合考虑EI与归因集中度，优先选择既具有改进潜力又能给出较清晰归因依据的候选解。这种采样策略有助于提高搜索的针对性，并为工程决策提供更明确的参考。在BEXEA中，EU准则用于筛选候选解，同时也被纳入赌博机奖励机制，引导搜索策略根据可解释性反馈进行调整。
...
本研究采用两组基准函数集。第一组为IEEE CEC 2022基准函数集（$d = 20$），用于考察算法在标准维度下的优化能力；第二组为高维场景基准函数集（$d = 50$和$d = 100$），涵盖单峰、多峰旋转和混合函数。统计分析采用Friedman检验与Wilcoxon秩和检验，显著性水平设为$\alpha = 0.05$。

对比算法包括TSA-BFEA、KTA2、DESO等七种SAEAs，其名称沿用原始文献中的缩写形式，参数采用原作者推荐设置。BEXEA的衰减因子设为0.3，近邻数设为8。所有任务的函数评估上限均为1000次，并独立运行20次。

\subsection{油藏问题描述}

本节将算法应用于油藏产量优化任务，目标是在权衡注水与生产成本的前提下寻找最优井控方案，以实现净现值（Net Present Value, NPV）最大化。
...
\bicaption{BEXEA与竞争算法在IEEE CEC 2022基准函数集上的比较结果（$d = 20$）}{Comparison result of BEXEA and its competitors on IEEE CEC 2022 benchmark function set with $d = 20$}
...
表\ref{t:bexea_CEC22_Table}显示，在CEC 2022基准函数集测试中，BEXEA的平均排名位居第一。Wilcoxon秩和检验表明，BEXEA在多数函数上占优，主要原因在于MAB机制能够实时调整策略权重，EU准则也通过特征归因分析提高了样本评估效率。

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.82\textwidth]{figures/chapter5/BEXEA-CEC22-CC.png}
\bicaption{BEXEA与竞争算法在IEEE CEC 2022基准函数集上的收敛曲线（$d = 20$）}{Convergence curves of BEXEA and its competitors on the IEEE CEC 2022 benchmark function set ($d = 20$)}
...
在高维场景（表\ref{t:bexea_CEC05_Table}）中，BEXEA仍保持较好的性能。在100维环境下，MAB驱动的策略切换机制能够较好地应对复杂函数特性，在维持种群多样性的同时降低陷入局部极值的风险，说明该方法具有良好的高维可扩展能力。

从计算开销角度看，BEXEA在每次真实评估前需要维护代理模型、计算局部不确定性并更新赌博机奖励，因此单次采样决策比固定搜索策略更复杂。但在昂贵优化场景中，真实函数评估通常占据主要时间成本，减少无效评估比单步决策开销更关键。高维基准测试结果表明，BEXEA在有限评估预算下仍能保持较好的收敛质量，说明其额外策略调度成本能够被样本效率提升所抵消。

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.92\textwidth]{figures/chapter5/BEXEA-Plots.png}
\bicaption{BEXEA与竞争算法在IEEE CEC 2022基准函数集上的散点图和排名图（$d = 20$）}{Combined scatter and rank plots of BEXEA and its competitors on the IEEE CEC 2022 benchmark function set ($d = 20$)}
...
\renewcommand{\arraystretch}{0.88}
\setlength{\tabcolsep}{3pt}
\bicaption{BEXEA与竞争算法在高维场景基准函数集上的比较结果（$d = 50$和$d = 100$）}{Comparison results of BEXEA and its competitors on the high-dimensional benchmark function set with $d = 50$ and $d = 100$}
...
\subsubsection{核心机制分析}

消融实验验证了自适应搜索机制的必要性。表\ref{t:bexea_MAB_Table}显示，BEXEA的整体性能优于各单一策略版本。由此可见，MAB驱动的灵活切换机制是算法适应不同问题特征并在多阶段维持较好性能的关键因素。
...
采样准则的消融分析见表\ref{t:bexea_EU_Table}。结果表明，缺少采样筛选环节会导致算法求解精度下降。EU准则与代理模型预测相结合后，在多项任务中表现出较好的互补性，整体排名也位居前列。这说明利用特征重要性归因引导采样决策，能够更有效地分配有限评估预算，并增强优化过程的可解释性。
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.90\textwidth]{figures/chapter5/beta.png}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.90\textwidth]{figures/chapter5/k.png}
...
本实验利用MATLAB油藏模拟工具箱（MATLAB Reservoir Simulation Toolbox, MRST）构建仿真环境，对井控问题开展数值仿真实验。
...
表\ref{t:bexea_Oil_Exp_Table}显示，BEXEA在核心经济指标上优于所有对比算法。其平均净现值（NPV）高于次优算法，同时标准差更小，说明求解结果具有较好一致性。从运行时间看，BEXEA的平均耗时为$2.1334\times10^4$，低于所有对比SAEAs，表明其在油藏产量优化中不仅提升了经济目标值，也减少了达到高质量方案所需的实际计算开销。

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.88\textwidth]{figures/chapter5/runBEXEA-OilReservoir-sampled-CC.png}
...
收敛轨迹（图\ref{fig:bexea_Oil_Exp_Figure}）表明，BEXEA在搜索初期便能较早获得有效改进。进入中后期后，MAB机制能够根据搜索停滞状态动态调整策略，使算法持续向更优区域逼近，并在有限评估预算内获得较优的经济收益。

\section{本章小结}
本章针对代理辅助昂贵优化中搜索策略自适应不足和采样准则缺乏解释性等问题，构建了BEXEA算法。该方法融合多臂赌博机自适应机制与可解释人工智能思想，实现了搜索资源分配与采样决策的协同调控。

实验结果表明，BEXEA在不同维度的标准基准函数集测试中均取得了较好的寻优结果。在油藏产量优化这一典型工程应用中，算法的收敛速度与求解精度进一步验证了其处理高维复杂昂贵优化问题的有效性。消融实验则从不同角度分析了自适应策略演化和可解释不确定性采样准则对整体性能的提升作用。BEXEA在增强搜索自适应性的同时，也通过特征归因分析提高了采样决策的可解释性，为评估代价较高的优化问题提供了一种可行的求解框架。

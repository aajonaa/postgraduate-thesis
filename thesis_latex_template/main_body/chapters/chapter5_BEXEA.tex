...
实验结果表明，BEXEA在不同维度的标准测试函数上均表现出较强的寻优能力。在油藏产量优化这一典型工程算例中，算法的收敛速度与求解精度进一步验证了其处理高维复杂工程问题的有效性。消融实验则从不同角度分析了自适应策略演化和多准则采样对整体性能的提升作用。BEXEA在增强搜索自适应性的同时，也通过特征归因分析提高了采样决策的可解释性，为昂贵优化问题提供了一种可行的求解思路。
